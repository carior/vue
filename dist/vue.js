...
(function (global, factory) {
  ...
  function simpleNormalizeChildren (children) {
    for (var i = 0; i < children.length; i++) {
      if (Array.isArray(children[i])) {
        ...
        // apply方法的第二个参数（一个数组，或类数组的对象）会作为被调用对象的arguments值，也就是说该数组的各个元素将会依次成为被调用函数的各个参数；
   ...
   * @param {string} [nestedIndex] 表示嵌套的索引，因为单个 child 可能是一个数组类型
   * @returns {Array<VNode>}
   */
  ...
  function normalizeArrayChildren (children, nestedIndex) {
    ...
    for (i = 0; i < children.length; i++) {
      ...
      last = res[lastIndex];
      //  nested 
      if (Array.isArray(c)) { // 1.如果是一个数组类型，则递归调用 normalizeArrayChildren
        if (c.length > 0) {
      ...
      } else if (isPrimitive(c)) { // 2.如果是基础类型 则通过 createTextVNode 方法转换成 VNode 类型；
        if (isTextNode(last)) {
          // merge adjacent text nodes
  ...
  /**
   * @export
   * @param {Component} context 表示vnode的上下文环境 它是一个Component类型
   * @param {(string | Class<Component> | Function | Object)} [tag] 表示标签，它可以是一个字符串，也可以是一个Component
   * @param {VNodeData} [data] 表示vnode的数据，可以在flow/vnode.js 中找到它的定义
   * @param {*} [children] 表示当前vnode的子节点，它是任意类型的，它接下来需要被规范为标准的vnode数组
   * @param {number} [normalizationType] 表示子节点规范的类型，类型不同规范的方法也就不一样，它主要参考render函数是编译生成的还是用户手写的
  ...
  function _createElement (
    ...
    // normalizeChildren 和 simpleNormalizeChildren 定义在src/core/vdom/helpers/normalzie-children.js 中
    ...
    if (typeof tag === 'string') {
      ...
      if (config.isReservedTag(tag)) { // 如果是内置的一些节点，则直接创建一个普通 VNode
  ...
  function createPatchFunction (backend) {
    ...
    function createElm (
      ...
      if (isDef(tag)) {
        ...
        /* istanbul ignore if */
        {
          ...
          if (isDef(data)) {
            // 调用invokeCreateHooks 方法执行所有的 create 的钩子并把 vnode push 到 insertedVnodeQueue 中
            invokeCreateHooks(vnode, insertedVnodeQueue);
          }
          // 调用insert 方法把 DOM 插入到父节点中，因为是递归调用，子元素会优先调用 insert，所以整个 vnode 树节点的插入顺序是先子后父。
    ...
    // 调用一些 nodeOps 把子节点插入到父节点中，这些辅助方法定义在 src/platforms/web/runtime/node-ops.js 中：
    ...
    // 最终返回了一个patch方法， 这个方法就赋值给了 vm._update 函数里调用的 vm.__patch__，在 src\core\instance\lifecycle.js 里
